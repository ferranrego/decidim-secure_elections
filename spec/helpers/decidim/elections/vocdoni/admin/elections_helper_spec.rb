    ...
    module Vocdoni
      module Admin
        describe ElectionsHelper do
          ...
          describe "#vocdoni_language_selector_label" do
            subject(:markup) { helper.vocdoni_language_selector_label("some-field-tabs", "Question title") }
          ...
          describe "#vocdoni_direct_voting_url" do
            subject(:url) { helper.vocdoni_direct_voting_url(election) }

            let(:election) { create(:vocdoni_election, :on_chain, component:) }

            it "is absolute, because it is going into somebody else's inbox" do
              expect(url).to start_with("http://test.host#{Decidim::Elections::Vocdoni::Engine::VOTE_PATH}?v=")
            end

            it "opens this election, with no way back and no credential" do
              fields = Decidim::Elections::Vocdoni::VotingPageUrl.decode(URI.parse(url).query.delete_prefix("v="))
