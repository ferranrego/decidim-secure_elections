    ...
    module Vocdoni
      module Admin
        ...
        module ElectionsHelper
          ...
          # `Decidim::Elections::Vocdoni::Engine` — Decidim 0.33 raises at render time on an
          ...
          def vocdoni_admin_nav_items(election)
            Decidim::Elections::Vocdoni::Election::NAV_STEPS.map do |step|
              blocker = election.step_blocker(step)
              number = Decidim::Elections::Vocdoni::Election::WIZARD_STEPS.index(step)&.succ

              NavItem.new(
                key: step,
                label: vocdoni_step_label(step),
                ...
                path: blocker.nil? ? vocdoni_step_path(election, step) : "#",
                number:,
                enabled: blocker.nil?,
          ...
          def vocdoni_step_progress(election, current)
            number = Decidim::Elections::Vocdoni::Election::WIZARD_STEPS.index(current.to_s.to_sym)&.succ
            return nil if number.blank?

            t("decidim.elections.vocdoni.admin.nav.progress",
              number:,
              total: Decidim::Elections::Vocdoni::Election::WIZARD_STEPS.size,
              completed: election.completed_steps_count)
          end

          # @param step [Symbol]
          # @return [String]
          def vocdoni_step_label(step)
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.menu.details")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.menu.questions")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.menu.census")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.menu.calendar")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.menu.publish")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.menu.monitor")
            t(step, scope: "decidim.elections.vocdoni.admin.menu")
          end

          # @param blocker [Symbol] as returned by `Election#step_blocker`
          # @return [String]
          def vocdoni_step_reason(blocker)
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.nav.reasons.unsaved")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.nav.reasons.not_on_chain")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.nav.reasons.locked_on_chain")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.nav.reasons.details_incomplete")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.nav.reasons.questions_incomplete")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.nav.reasons.census_incomplete")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.nav.reasons.calendar_incomplete")
            t(blocker, scope: "decidim.elections.vocdoni.admin.nav.reasons")
          end
          ...
          def vocdoni_question_type_options
            # i18n-tasks-use t("decidim.elections.vocdoni.question_types.singlechoice")
            # i18n-tasks-use t("decidim.elections.vocdoni.question_types.multichoice")
            Decidim::Elections::Vocdoni::Question::QUESTION_TYPES.map do |type|
              [t(type, scope: "decidim.elections.vocdoni.question_types"), type]
            end
          end
          ...
          def vocdoni_language_selector_label(tabs_id, field)
            return unless vocdoni_language_selector_is_a_select?

            label_tag tabs_id,
                      t("decidim.elections.vocdoni.admin.elections.editor.language_selector_label", field:),
                      class: "sr-only"
          end
          ...
          # (`decidim.elections.vocdoni.elections.states`), so an administrator and a
          ...
          # useful.
          def vocdoni_election_status_label(election)
            state = election.display_state

            content_tag(:span,
                        vocdoni_display_state_label(state),
                        class: "label #{DISPLAY_STATE_CLASSES.fetch(state, "basic")}")
          end

          # @param state [Symbol] as returned by `Election#display_state`
          # @return [String]
          def vocdoni_display_state_label(state)
            # i18n-tasks-use t("decidim.elections.vocdoni.elections.states.draft")
            # i18n-tasks-use t("decidim.elections.vocdoni.elections.states.publishing")
            # i18n-tasks-use t("decidim.elections.vocdoni.elections.states.scheduled")
            # i18n-tasks-use t("decidim.elections.vocdoni.elections.states.ongoing")
            # i18n-tasks-use t("decidim.elections.vocdoni.elections.states.paused")
            # i18n-tasks-use t("decidim.elections.vocdoni.elections.states.ended")
            # i18n-tasks-use t("decidim.elections.vocdoni.elections.states.results")
            # i18n-tasks-use t("decidim.elections.vocdoni.elections.states.canceled")
            t(state, scope: "decidim.elections.vocdoni.elections.states")
          end
          ...
          def vocdoni_upstream_status_label(election)
            # i18n-tasks-use t("decidim.elections.vocdoni.statuses.draft")
            t(election.status, scope: "decidim.elections.vocdoni.statuses", default: election.status.to_s)
          end
          ...
          def vocdoni_start_time_text(election)
            ...
            return t("decidim.elections.vocdoni.admin.elections.editor.started_on_publication") if election.on_chain?
            return t("decidim.elections.vocdoni.admin.elections.form.manual_start") if election.manual_start?

            t("decidim.elections.vocdoni.admin.elections.editor.starts_on_publication")
          end
          ...
          def vocdoni_last_synced_text(election)
            return t("decidim.elections.vocdoni.admin.monitor.show.never_synced") if election.results_synced_at.blank?

            t("decidim.elections.vocdoni.admin.monitor.show.last_synced",
          ...
          def vocdoni_status_confirm_data(status)
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.show.confirm.ready.body")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.show.confirm.ready.title")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.show.confirm.paused.body")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.show.confirm.paused.title")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.show.confirm.ended.body")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.show.confirm.ended.title")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.show.confirm.canceled.body")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.show.confirm.canceled.title")
            scope = "decidim.elections.vocdoni.admin.monitor.show.confirm.#{status}"
          ...
          def vocdoni_trash_confirm_data(election)
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.elections.confirm_trash.draft.body")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.elections.confirm_trash.draft.title")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.elections.confirm_trash.on_chain.body")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.elections.confirm_trash.on_chain.title")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.elections.confirm_trash.voting_open.body")
            # i18n-tasks-use t("decidim.elections.vocdoni.admin.elections.confirm_trash.voting_open.title")
            scope = "decidim.elections.vocdoni.admin.elections.confirm_trash.#{vocdoni_trash_confirm_key(election)}"
          ...
          # @param question [Decidim::Elections::Vocdoni::Question]
          # @return [Symbol] a key of `decidim.elections.vocdoni.elections.states`
          def vocdoni_question_display_state(question)
            ...
            Decidim::Elections::Vocdoni::ElectionStatusCell.state_for(question.election)
          end

          def vocdoni_turnout_text(election)
            return t("decidim.elections.vocdoni.admin.monitor.turnout_unknown") if election.turnout.nil?

            t("decidim.elections.vocdoni.admin.monitor.turnout_value",
          ...
          def vocdoni_direct_voting_url(election)
            path = Decidim::Elections::Vocdoni::VotingPageUrl.build(election, locale: I18n.locale)
          ...
          def vocdoni_locked_reason(election)
            ...
            t("decidim.elections.vocdoni.admin.nav.locked_on_chain")
          end
          ...
          def vocdoni_language_selector_is_a_select?
