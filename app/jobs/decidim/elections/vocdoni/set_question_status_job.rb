    ...
    module Vocdoni
      ...
      class SetQuestionStatusJob < ApplicationJob
        ...
        def perform(election_id, status, question_ids = nil)
          ...
          return unless Decidim::Elections::Vocdoni::Question::STATUSES.include?(@status)
        ...
        def persist_locally!
          Decidim::Elections::Vocdoni::Election.transaction do
            Decidim::Elections::Vocdoni::Question.where(id: questions.map(&:id)).update_all(vocdoni_status: status, updated_at: Time.current) # rubocop:disable Rails/SkipsModelValidations

            next unless whole_process?
            next unless Decidim::Elections::Vocdoni::Election::STATUSES.include?(status)
