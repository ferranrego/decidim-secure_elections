      ...
      module Admin
        ...
        class DashboardController < Admin::ApplicationController
          ...
          def show
            ...
            @form = form(Decidim::Elections::Vocdoni::Admin::SetupForm).instance(election:) if election.editable?
          ...
          def publish
            ...
            @form = form(Decidim::Elections::Vocdoni::Admin::SetupForm).from_params(params, election:)
            ...
            @form ||= form(Decidim::Elections::Vocdoni::Admin::SetupForm).instance(election:)
            ...
            if @form.valid?
              ...
              flash[:notice] = I18n.t("setup.create.success", scope: "decidim.elections.vocdoni.admin")
              redirect_to election_dashboard_path(election)
            else
              flash.now[:alert] = I18n.t("setup.create.invalid", scope: "decidim.elections.vocdoni.admin")
          ...
          def unpublish
            ...
            Decidim::Elections::Vocdoni::Admin::UnpublishElection.call(election, current_user) do
              on(:ok) do
                flash[:notice] = I18n.t("elections.unpublish.success", scope: "decidim.elections.vocdoni.admin")
                redirect_to election_dashboard_path(election)
              end

              on(:invalid) do
                flash[:alert] = I18n.t("elections.unpublish.invalid", scope: "decidim.elections.vocdoni.admin")
                redirect_to election_dashboard_path(election)
              end
            end
          end
          ...
          def start
            ...
            @form = form(Decidim::Elections::Vocdoni::Admin::QuestionStatusForm).from_params(
              { question_status: { status: "ready" } },
              election:
            )

            Decidim::Elections::Vocdoni::Admin::UpdateQuestionStatus.call(@form, current_user) do
              on(:ok) do
                flash[:notice] = I18n.t("dashboard.start.success", scope: "decidim.elections.vocdoni.admin")
                redirect_to election_dashboard_path(election)
              end

              on(:invalid) do
                flash[:alert] = I18n.t("dashboard.start.invalid", scope: "decidim.elections.vocdoni.admin")
                redirect_to election_dashboard_path(election)
              end
            end
          end
          ...
          def refresh
            ...
            Decidim::Elections::Vocdoni::Admin::RefreshElection.call(election) do
              on(:ok) do |mode|
                # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.refresh.publish")
                # i18n-tasks-use t("decidim.elections.vocdoni.admin.monitor.refresh.results")
                enqueued = true
                message = I18n.t("monitor.refresh.#{mode}", scope: "decidim.elections.vocdoni.admin")
              end

              on(:invalid) do
                message = I18n.t("monitor.refresh.invalid", scope: "decidim.elections.vocdoni.admin")
              end
            end
          ...
          def status
            ...
            @form = form(Decidim::Elections::Vocdoni::Admin::QuestionStatusForm).from_params(params, election:)

            Decidim::Elections::Vocdoni::Admin::UpdateQuestionStatus.call(@form, current_user) do
              on(:ok) do
                flash[:notice] = I18n.t("monitor.status.success", scope: "decidim.elections.vocdoni.admin")
                redirect_to election_dashboard_path(election)
              end

              on(:invalid) do
                flash.now[:alert] = I18n.t("monitor.status.invalid", scope: "decidim.elections.vocdoni.admin")
                render action: "show", status: :unprocessable_content
              end
            end
          end
          ...
          def monitor_payload
            {
              ...
              state_label: helpers.vocdoni_display_state_label(election.display_state),
              ...
              synced_text: helpers.vocdoni_last_synced_text(election),
          ...
          def question_payload(question)
            {
              id: question.id,
              votes_text: t("decidim.elections.vocdoni.admin.monitor.votes_count", count: question.votes_count),
          ...
          def answer_payload(answer)
            {
              id: answer.id,
              votes_text: t("decidim.elections.vocdoni.admin.monitor.votes_count", count: answer.votes_count.to_i),
