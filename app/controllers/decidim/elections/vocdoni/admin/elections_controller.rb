      ...
      module Admin
        ...
        class ElectionsController < Admin::ApplicationController
          ...
          def new
            ...
            @form = form(Decidim::Elections::Vocdoni::Admin::ElectionForm).instance
          end

          def create
            enforce_permission_to :create, :election

            @form = form(Decidim::Elections::Vocdoni::Admin::ElectionForm).from_params(params, current_component:)

            Decidim::Elections::Vocdoni::Admin::CreateElection.call(@form) do
              on(:ok) do |election|
                flash[:notice] = I18n.t("elections.create.success", scope: "decidim.elections.vocdoni.admin")
              ...
              on(:invalid) do
                flash.now[:alert] = I18n.t("elections.create.invalid", scope: "decidim.elections.vocdoni.admin")
          ...
          def edit
            ...
            @form = form(Decidim::Elections::Vocdoni::Admin::ElectionForm).from_model(election, election:)
          end

          def update
            enforce_permission_to(:update, :election, election:)

            @form = form(Decidim::Elections::Vocdoni::Admin::ElectionForm).from_params(params, current_component:, election:)

            Decidim::Elections::Vocdoni::Admin::UpdateElection.call(@form, election) do
              on(:ok) do
                flash[:notice] = I18n.t("elections.update.success", scope: "decidim.elections.vocdoni.admin")
                redirect_to edit_election_path(election)
              end

              on(:invalid) do
                flash.now[:alert] = I18n.t("elections.update.invalid", scope: "decidim.elections.vocdoni.admin")
          ...
          def publish
            ...
            Decidim::Elections::Vocdoni::Admin::PublishElection.call(election, current_user) do
              on(:ok) do
                ...
                # i18n-tasks-use t("decidim.elections.vocdoni.admin.elections.publish.success")
                # i18n-tasks-use t("decidim.elections.vocdoni.admin.elections.publish.success_not_on_chain")
                key = election.on_chain? ? "success" : "success_not_on_chain"
                flash[:notice] = I18n.t("elections.publish.#{key}", scope: "decidim.elections.vocdoni.admin")
                redirect_to elections_path
              end

              on(:invalid) do
                flash[:alert] = I18n.t("elections.publish.invalid", scope: "decidim.elections.vocdoni.admin")
                redirect_to elections_path
              end
            end
          end
          ...
          def collection
            @collection ||= Decidim::Elections::Vocdoni::Election.where(component: current_component)
          end
          ...
          def trashable_deleted_resource
            @trashable_deleted_resource ||= Decidim::Elections::Vocdoni::Election.with_deleted.find_by(component: current_component, id: params[:id])
          end
        end
      end
    end
