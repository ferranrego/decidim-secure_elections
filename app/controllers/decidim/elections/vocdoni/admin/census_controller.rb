      ...
      module Admin
        ...
        class CensusController < Admin::ApplicationController
          ...
          def update
            ...
            @form = form(Decidim::Elections::Vocdoni::Admin::CensusForm).from_params(params, election:)

            Decidim::Elections::Vocdoni::Admin::UpdateElectionCensus.call(@form, election) do
              on(:ok) do
                respond_to do |format|
                  format.html do
                    flash[:notice] = I18n.t("census.authentication.success", scope: "decidim.elections.vocdoni.admin")
              ...
              on(:invalid) do
                respond_to do |format|
                  format.html do
                    flash.now[:alert] = I18n.t("census.authentication.invalid", scope: "decidim.elections.vocdoni.admin")
          ...
          def update_members
            ...
            @form = form(Decidim::Elections::Vocdoni::Admin::CensusMembersForm).from_params(params, election:)

            Decidim::Elections::Vocdoni::Admin::UpdateCensusMembers.call(@form, election, current_user) do
              on(:ok) do
                flash[:notice] = I18n.t("census.members.update.success", scope: "decidim.elections.vocdoni.admin")
                redirect_to election_census_path(election)
              end

              on(:invalid) do
                flash.now[:alert] = I18n.t("census.members.update.invalid", scope: "decidim.elections.vocdoni.admin")
          ...
          def template
            ...
            csv_template = Decidim::Elections::Vocdoni::CensusCsv::Template.new(template_fields)

            if csv_template.any?
              send_data csv_template.to_csv, filename: csv_template.filename, type: "text/csv; charset=utf-8"
            else
              flash[:alert] = I18n.t("census.template.no_columns", scope: "decidim.elections.vocdoni.admin")
              redirect_to election_census_path(election)
            end
          end
          ...
          def import
            ...
            @import_form = form(Decidim::Elections::Vocdoni::Admin::CensusImportForm).from_params(params, election:)
            ...
            Decidim::Elections::Vocdoni::Admin::ImportCensusMembers.call(@import_form, election, current_user) do
              on(:ok) do |result|
          ...
          def import_from_verifications
            ...
            @verifications_form = form(Decidim::Elections::Vocdoni::Admin::CensusVerificationsForm)
            ...
            Decidim::Elections::Vocdoni::Admin::ImportCensusMembersFromVerifications.call(@verifications_form, election, current_user) do
              on(:ok) do |imported|
                flash[:notice] = I18n.t("census.verifications.success", scope: "decidim.elections.vocdoni.admin", count: imported)
                redirect_to election_census_path(election)
              end
          ...
          def clear
            ...
            Decidim::Elections::Vocdoni::Admin::DestroyCensusMembers.call(election, current_user) do
              on(:ok) do |count|
                flash[:notice] = I18n.t("census.clear.success", scope: "decidim.elections.vocdoni.admin", count:)
                redirect_to election_census_path(election)
              end

              on(:invalid) do
                flash[:alert] = I18n.t("census.clear.invalid", scope: "decidim.elections.vocdoni.admin")
                redirect_to election_census_path(election)
              end
            end
          end
          ...
          def census_form
            form(Decidim::Elections::Vocdoni::Admin::CensusForm).from_model(election, election:)
          end
          ...
          def members_form
            form(Decidim::Elections::Vocdoni::Admin::CensusMembersForm).from_model(election, election:).tap do |built|
              built.members << blank_member_form
            end
          end

          def blank_member_form
            form(Decidim::Elections::Vocdoni::Admin::CensusMemberForm).from_params({}, election:)
          end

          def import_form
            @import_form ||= form(Decidim::Elections::Vocdoni::Admin::CensusImportForm).from_params({}, election:)
          end
          ...
          def available_handlers
            @available_handlers ||= Decidim::Elections::Vocdoni::VerifiedParticipants.available_handlers(current_organization)
          end
          ...
          def template_fields
            @template_fields ||= begin
              requested = Array(params[:fields]).map(&:to_s) & Decidim::Elections::Vocdoni::CensusMember::FIELDS
          ...
          def import_failure_message(result)
            ...
            form_errors_message(@import_form).presence ||
              I18n.t("census.import.invalid", scope: "decidim.elections.vocdoni.admin")
          end

          # The template's example row, recognised and left out rather than
          # imported as a person
          # ({Decidim::Elections::Vocdoni::CensusCsv::Importer#example_row?}).
          #
          ...
          def skipped_examples_message(result)
            ...
            I18n.t("census.import.skipped_example", scope: "decidim.elections.vocdoni.admin", count: result.skipped_examples_count)
          end
          ...
          def import_success_message(result)
            added = I18n.t("census.import.success", scope: "decidim.elections.vocdoni.admin", count: result.imported_count)
            removed = I18n.t("census.import.replaced", scope: "decidim.elections.vocdoni.admin", count: result.removed_count) if result.any_removed?
          ...
          def verifications_failure_message
            form_errors_message(@verifications_form).presence ||
              I18n.t("census.verifications.invalid", scope: "decidim.elections.vocdoni.admin")
          end
          ...
          def failed_rows_message(result)
            ...
            shown = result.failed_rows.first(10).map do |row|
              I18n.t("census.import.failed_row", scope: "decidim.elections.vocdoni.admin", number: row.number, reason: row.summary)
            end

            remaining = result.failed_count - shown.size
            shown << I18n.t("census.import.more_failures", scope: "decidim.elections.vocdoni.admin", count: remaining) if remaining.positive?

            I18n.t("census.import.failures", scope: "decidim.elections.vocdoni.admin", count: result.failed_count, rows: shown.join(" "))
          end
        end
      end
    end
